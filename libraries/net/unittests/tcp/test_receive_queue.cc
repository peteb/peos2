...
TESTSUITE(net::tcp::receive_queue) {
  TESTCASE("readable_until returns 0 when queue is empty") {
    net::tcp::receive_queue q;
  ...
  TESTCASE("readable_until returns the size of the segment at first seq nbr") {
    ...
    // when
    q.insert(net::tcp::tcp_recv_segment{0, 0, 0}, data, 256);
  ...
  TESTCASE("readable_until returns 0 if there's a gap to the first segment") {
    ...
    // when
    q.insert(net::tcp::tcp_recv_segment{0, 10, 0}, data, 256);
  ...
  TESTCASE("read_one_segment reassembles segments correctly") {
    // given
    net::tcp::receive_queue q;
    ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, 2, 0}, "BB", 2), true);
    ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, 7, 0}, "DD", 2), true);
    ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, 0, 0}, "AA", 2), true);
    ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, 4, 0}, "CCC", 3), true);
    ...
    net::tcp::tcp_recv_segment segment;
  ...
  TESTCASE("read_one_segment and insert can read/write many times") {
    ...
    for (int i = 0; i < 30000; ++i) {
      ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, seqnbr, 0}, bs, sizeof(bs)), true);
      seqnbr += sizeof(bs);
      ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, seqnbr, 0}, as, sizeof(as)), true);
      seqnbr += sizeof(as);
  ...
  TESTCASE("32 bit wrap-around: segments can be written over the border") {
    ...
    // when
    net::tcp::tcp_recv_segment segment;
    ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, 0xFFFFFFF0, 0}, data, 20), true);
    ASSERT_EQ(q.readable_until(), 4u);
    ASSERT_EQ(q.read_one_segment(&segment, data, sizeof(data)), true);
    ASSERT_EQ(segment.seqnbr, 0xFFFFFFF0);
    ASSERT_EQ(segment.length, 20u);

    ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, 4u, 0}, data, 20), true);
  ...
  TESTCASE("32 bit wrap-around: segments are accepted when reader is lagging") {
    ...
    ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, 0xFFFFFFF0, 0}, data, 20), true);
    ASSERT_EQ(q.readable_until(), 4u);
    ASSERT_EQ(q.insert(net::tcp::tcp_recv_segment{0, 4u, 0}, data, 1000), true);
  }
}
