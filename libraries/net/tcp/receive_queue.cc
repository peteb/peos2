...
namespace net::tcp {
  bool receive_queue::insert(const tcp_recv_segment &segment, const char *data, size_t length)
  {
    if (_segments.full())
      return false;
  ...
  bool receive_queue::read_one_segment(tcp_recv_segment *segment,
                                        char *buffer,
                                        size_t len)
  {
  ...
  net::tcp::sequence_number receive_queue::readable_until() const
  {
    if (_segments.size() == 0)
      return _read_cursor;
  ...
  bool receive_queue::has_readable() const
  {
    if (_segments.size() == 0)
      return false;
  ...
  net::tcp::sequence_number receive_queue::read_cursor() const
  {
    return _read_cursor;
  }

  size_t receive_queue::find_front_segment() const
  {
  ...
  void receive_queue::reset(net::tcp::sequence_number seqnbr)
  {
    _read_cursor = seqnbr;
    _data_buffer.clear();
    _segments.clear();
