...
extern "C" int init_main(int argc, const char **argv)
{
  ...
  if (child_pid == 0) {
    ...
    const char *argv[] = {init_command, command_line, nullptr};
...
{
  puts_sys(kernout, p2::format<128>("init: writing /ramfs%s%s\n", path, filename));
...
void load_multiboot_modules()
{
  ...
  puts_sys(kernout, p2::format<64>("init: mods_count: %d\n", (uint32_t)mbhdr->mods_count));
  puts_sys(kernout, p2::format<128>("init: cmd_line: \"%s\"\n", (const char *)PHYS2KERNVIRT(mbhdr->cmd_line)));
